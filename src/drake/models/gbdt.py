...
from drake.data.features import CONTEXT_COLUMNS, DRAFT_COLUMNS, GAME_STATE_COLUMNS, LABEL_COLUMN
...
class GbdtBaseline(IWinProbabilityModel):
    ...
    def fit(self, train: pd.DataFrame, val: pd.DataFrame) -> None:
        ...
        self._draft_booster = self._train_booster(
            _draft_matrix(train[train["timestep"] == 0]),
            _draft_matrix(val[val["timestep"] == 0]),
        )
    ...
    def predict(self, rows: pd.DataFrame) -> NDArray[np.float64]:
        ...
        if draft_mask.any():
            features, _ = _draft_matrix(rows[draft_mask])
    ...
    def _with_draft_probability(self, rows: pd.DataFrame) -> pd.DataFrame:
        ...
        features, _ = _draft_matrix(rows)
        enriched = rows.copy()
        enriched[DRAFT_PROBABILITY_COLUMN] = np.asarray(self._draft_booster.predict(features))
        return enriched


def _draft_matrix(rows: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series]:
    ...
    for column in DRAFT_COLUMNS:
        # Fixed categories keep LightGBM's category codes stable between fit and predict.
        features[column] = pd.Categorical(features[column], categories=range(UNKNOWN_CHAMPION_INDEX + 1))
